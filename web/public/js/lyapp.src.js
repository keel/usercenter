...
$.fn.serializeJson = function(removeEmpty) {
  var o = {};
  ...
  $.each(a, function() {
    if (o[this.name] !== undefined) {
      ...
      if (!removeEmpty || this.value) {
        o[this.name] = this.value || '';
      }
    }
  });
  // console.log('form json:', o);
  return o;
};

var formToJson = function(formDom, removeEmpty) {
  // console.log('form:', formDom);
  return $(formDom).serializeJson(removeEmpty);
};
var jsonReq = function(url, data, contentType, callback) {
  if (!callback && Object.prototype.toString.call(contentType) === '[object Function]') {
...
var asDom = function(content, domName) {
...
var showTable = function(titleMap, rowJson, cellFnMap) {
  ...
  if (titleOrder) {
    for (var i = 0; i < titleOrder.length; i++) {
      tb += asDom(titleMap[titleOrder[i]], 'th');
    }
  } else {
    titleOrder = [];
    for (var key in titleMap) {
      tb += asDom(titleMap[key], 'th');
      titleOrder.push(key);
    }
  }
  tb += '</thead><tbody>';
  for (var j = 0; j < rowJson.length; j++) {
    tb += '<tr>';
    var rowObj = rowJson[j];
    for (var k = 0; k < titleOrder.length; k++) {
      var cellKey = titleOrder[k];
      var content = rowObj[cellKey];
      if (cellFnMap) {
        var cellFn = cellFnMap[cellKey];
        content = cellFn ? cellFn(cellKey, rowObj[cellKey]) : rowObj[cellKey];
      }
      tb += asDom(content, 'td');
    }
    tb += '</tr>';
